  ...
  <head>
	...
	<script type="text/javascript">
			...
			$(function() {
				 $dg = $("#dg");
				 $grid=$dg.datagrid({
					url : "<%=request.getContextPath()%>/queryMongoList.do",
					width : 'auto',
					...
					striped:true,
					singleSelect:true,
					columns : [ [ {field : 'operloginId',title : '操作用户',width : parseInt($(this).width()*0.1)},
					              {field : 'computerIp',title : 'IP地址',width : parseInt($(this).width()*0.1)},
					              {field : 'computerName',title : '物理地址',width : parseInt($(this).width()*0.1)},
					              {field : 'opertime',title : '日志日期',width : parseInt($(this).width()*0.1)},
					              {field : 'opercontroller',title : '日志类型',width : parseInt($(this).width()*0.1),align : 'left',formatter:function(value,row){
				            		  if("1"==row.type)
											return "<font color=green>安全日志<font>";
					            		  else
					            			return "<font color=red>操作日志<font>";  
									}},
					              {field : 'operMethod',title : '操作名称',width :parseInt($(this).width()*0.1),align : 'left'},
					              {field : 'tomcatport',title : '端口号',width :parseInt($(this).width()*0.1),align : 'left'},
					              {field : 'operURI',title : '操作描述',width : parseInt($(this).width()*0.2),align : 'left'}
					              ] ],toolbar:'#tb'
				});
				//搜索框
				$("#searchbox").searchbox({ 

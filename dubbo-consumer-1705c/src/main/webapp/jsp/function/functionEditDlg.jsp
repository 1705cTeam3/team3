...
// 		下拉树替换	 lxx 2017年11月28日 11:44:03	
		 $('#selTree').combotree({ 
		 	url: '<%=request.getContextPath()%>/getSelTreeList.do',
		 	width:171,
		 	editable:true,//输入是否允许输入
		 	/* required: true,
		 	cascaseCheck : true,
		 	onlyLeafCheck:false, 
		    multiple : true // 是否多选 */
		...
		$("#form").form({
			url :"<%=request.getContextPath()%>/addInfo.do",
			/* onSubmit : function() {
				...
				<input name="state" id="state"  type="hidden"/>
				 <table>
					 <tr>
						...
						<th>父程式名称</th>
						<td>
						 	<input name="pid"  class="easyui-textbox" id="selTree" type="text"/>
							<input name="pname" id="pname"  type="hidden"/>
<!-- 							<select id="selTree" class="easyui-combotree" style="width:200px;"></select> -->
						</td>
